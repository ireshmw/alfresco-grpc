...
:build_share
    ...
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f alf-test-share
	call %MVN_EXEC% clean package -pl alf-test-share,alf-test-share-docker
EXIT /B 0
:build_acs
    docker-compose -f "%COMPOSE_FILE_PATH%" kill alf-test-acs
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f alf-test-acs
	call %MVN_EXEC% clean package -pl alf-test-integration-tests,alf-test-platform,alf-test-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl alf-test-platform,alf-test-integration-tests,alf-test-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl alf-test-platform,alf-test-integration-tests
EXIT /B 0
:purge
